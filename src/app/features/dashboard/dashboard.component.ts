...
/** Painel inicial - mostra as estatísticas de voucher (distribuição por status, totais, top
 *  clientes) pra quem tem VOUCHERS_CONSULT (mesmo conteúdo do antigo VoucherDashboardComponent,
 *  em `/voucher/dashboard`, movido pra cá por já ser o destino natural do login); quem não tem essa
 *  permissão continua vendo o estado vazio original. */
@Component({
  standalone: true,
...
export class DashboardComponent {
  ...
  private readonly voucherPolicy = inject(VoucherPermissionPolicy);

  readonly i18n = inject(I18nService);
  readonly facade = inject(VoucherStatisticsFacade);

  readonly canViewVoucherStats = computed(() => this.voucherPolicy.canView());
